...
def test_list_users_returns_accounts(client):
    """GET /v1/users returns list of accounts from the database."""
...
def test_list_users_empty(client):
    """GET /v1/users returns empty list when no accounts exist."""
    with patch("app.v1_routes.db") as mock_db:
...
def test_create_user_success(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        mock_db.execute_query.return_value = []  # no duplicate
...
def test_create_user_duplicate_email(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        mock_db.execute_query.return_value = [_ACCOUNT_ROW]  # duplicate found
        r = client.post(
            "/v1/users",
            json={
                "name": "Alice",
...
def test_update_user_not_found(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        mock_db.execute_query.return_value = []
        r = client.put(
            "/v1/users/00000000-0000-0000-0000-000000000099",
            json={"name": "Ghost"},
        )
    assert r.status_code == 404
...
def test_create_tenant_success(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        mock_db.execute_query.return_value = []  # no duplicate slug
        mock_db.execute_mutation.return_value = _TENANT_ROW
        r = client.post(
            "/v1/tenants",
            json={"name": "Acme", "description": "Acme Corp", "slug": "acme"},
        )
    assert r.status_code == 201
    assert r.json()["slug"] == "acme"
...
def test_create_tenant_duplicate_slug(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        ...
        r = client.post(
            "/v1/tenants",
            json={"name": "Acme2", "description": "", "slug": "acme"},
        )
    assert r.status_code == 409
...
def test_update_tenant_not_found(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        mock_db.execute_query.return_value = []
        r = client.put(
            "/v1/tenants/00000000-0000-0000-0000-000000000099",
            json={"name": "Ghost"},
        )
    assert r.status_code == 404
...
def test_list_tenant_members_found(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        mock_db.execute_query.side_effect = [
            ...
            [_TENANT_ACCOUNT_ROW],    # list members
        ]
...
def test_add_tenant_member_success(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        mock_db.execute_query.side_effect = [
            [_TENANT_ROW],     # _assert_tenant_exists
            [_ACCOUNT_ROW],    # _assert_account_exists
...
def test_add_tenant_member_duplicate(client):
    ...
    with patch("app.v1_routes.db") as mock_db:
        mock_db.execute_query.side_effect = [
            [_TENANT_ROW],         # _assert_tenant_exists
            [_ACCOUNT_ROW],        # _assert_account_exists
            [_TENANT_ACCOUNT_ROW], # already a member
        ]
        r = client.post(
            f"/v1/tenants/{_TENANT_ROW['id']}/members",
            json={
